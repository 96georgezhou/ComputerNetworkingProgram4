...
int main(int argc, char* argv[])
{
  ...
  while (true)
  {
    ...
    // 2. Spawn a child process through fork(). The parent closes this connection and goes back to 
    // the top of the loop, whereas the child continues checking the integrity of this connection.
    if (fork() == 0)
    { // Multithreading
      close(serverSD);
      ...
      // Get the client IP address and Port Number in a right format
      ...
      else
      {
        ...
        // if the address is listed in the hostent, then we can trust this connection
